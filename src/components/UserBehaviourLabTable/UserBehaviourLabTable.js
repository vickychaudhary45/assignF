...
import "./UserBehaviourLabTable.scss";

const UserBehaviourLabTable = () => {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const LabReportData = async () => {
    ...
    if (loginRes?.data?.auth_token) {
      let token = loginRes.data.auth_token;
      setLab_token(token);
      const data1 = {
      ...
      const reportRes = await getUserLabBehaviourReport(data1);
      if (reportRes?.data?.status === true) {
        setReports(reportRes.data.data.data);
        setTotalRows(reportRes.data.data.pagination.total * 1);
        setLoading(false);
      }
  ...
  useEffect(async () => {
    ...
    if (lab_token) {
      const res = await getTaskNames(data);
  ...
  const onSubmit = async () => {
    ...
    const res = await getUserLabBehaviourReport(requestData);
  ...
  const dataBarGoogle = [
    ["Lab Attempt Behaviour", "Count", { role: "style" }],
  ...
  const optionsPie = {
    title: "User Behaviour Lab Analytics Pie Chart",
  ...
  const optionsBar = {
    chart: {
      title: "User Behaviour Lab Analytics Bar Chart",
      colors: ["#dc3912", "#ff9900", "#FFD700", "#3366cc", "#109618"],
    },
  };

  return (
    <div className="user-behaviour-lab-block">
      <h3>User Behaviour Lab Analytics Report</h3>
      {reports?.length > 0 && (
              ...
              <TableBody>
                {reports?.length > 0 ? (
                  ...
                  <TableRow style={{ height: 53 * emptyRows }}>
                    <TableCell className="tab-no-data" colSpan={10}>
                      No Result Found.
                    </TableCell>
                  </TableRow>
                )}
...
export default UserBehaviourLabTable;
