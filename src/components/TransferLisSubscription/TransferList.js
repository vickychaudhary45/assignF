...
const TransferList = (props) => {
  ...
  const handleLeftToggle = (value, type) => () => {
    ...
    let isSelected = assignCourse?.some((item) => { return item.lms_subscription_id == value.lms_subscription_id }
    )
    if (isSelected) {
      course = assignCourse?.filter((item) => {
        return !(item.lms_subscription_id == value.lms_subscription_id)
      })
    } else {
      course.push(value)
    }
  ...
  const handleRightToggle = (value, type) => () => {
    ...
    let isSelected = assignRCourse?.some((item) => { return item.lms_subscription_id == value.lms_subscription_id })
    if (isSelected) {
      course = assignRCourse?.filter((item) => { return !(item.lms_subscription_id == value.lms_subscription_id) })
    } else {
      course.push(value)
    }
  ...
  const handleCheckedRight = async () => {
    ...
    courses?.map((item) => {
      ...
      assignCourse?.map((value, index) => {
        if (value.lms_subscription_id == item.lms_subscription_id) {
          isSelected = true;
        }
      })
      if (!isSelected) {
    ...
    <>
      ...
      <div className="inner-box">
        <CardHeader sx={{ px: 2, py: 1 }} title={title}
          subheader={`${title == 'All Subscriptions' ? assignCourse?.length : assignRCourse?.length}/${title == 'All Subscriptions' ? courses?.length : selectedCourses?.length
        ...
        <List dense component="div" role="list" sx={{ height: 282, bgcolor: "background.paper", overflow: "auto" }}>
          {items.length > 0 && items?.map((value) => {
            const labelId = `transfer-list-all-item-${value?.lms_subscription_id}-label`;
            return (
              <ListItem key={value.lms_subscription_id} role="listitem" button
                onClick={title == 'All Subscriptions' ? handleLeftToggle(value, title) : handleRightToggle(value, title)}
              >
                <ListItemIcon>
                  <Checkbox
                    checked={
                      title == 'All Subscriptions' ? assignCourse?.some(item => item.lms_subscription_id == value.lms_subscription_id) :
                        assignRCourse?.some(item => item.lms_subscription_id == value.lms_subscription_id)
                    }
                    tabIndex={-1}
                    disableRipple
                    inputProps={{
