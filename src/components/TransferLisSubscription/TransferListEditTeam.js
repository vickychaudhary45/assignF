...
const TransferList = (props) => {
  const { selectedCourses, setSelectedCourses, courses, setCourses, requestSearch, defaultSelectedBundles, setOpen, setSeverity, setMessage } = props;
  ...
  const handleRightToggle = (value, type) => () => {
    ...
    if (defaultSelectedBundles.includes(value.id)) {
      const alertObj = {
        status: "warning",
        message: "Already selected subscriptions can't be removed.",
      }
      setOpen(true);
