...
const TransferList = (props) => {
  const { selectedUsers, setSelectedUsers, users, setUsers, setSearchedUser, searchedUser } = props;
    ...
    <>
      <div className="inner-box">
        <div className="row">
          ...
          <div className="col-md-10">
            <CardHeader sx={{ px: 2, py: 1 }} title={title}
              subheader={`${title == 'All Users' ? assignCourse?.length : assignRCourse?.length}/${title == 'All Users' ? users?.length : selectedUsers?.length
                } users Selected`}
            />
          </div>
        </div>
        <div className="search-bar">
          {title == 'All Users' ?
            <input type="text" placeholder="Search users and add" onChange={(e) => { setSearchedUser(e.target.value) }} />
            ...
            return (
              ...
              >
                <ListItemIcon>
                  <Checkbox
                    checked={
                      title == 'All Users' ? assignCourse?.some(item => item.id == value.id && item.type == value.type) :
