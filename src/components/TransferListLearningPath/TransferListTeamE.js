...
const TransferList = (props) => {
  const { selectedLP, setSelectedLP, learningPath, setLearningPath, requestSearch, defaultSelectedLearningPath = [], setOpen, setSeverity, setMessage } = props;
  ...
  const handleRightToggle = (value, type) => () => {
    ...
    if (defaultSelectedLearningPath.includes(value.id)) {
      const alertObj = {
        status: "warning",
        message: "Already selected learning path can't be removed.",
      }
      setOpen(true);
        ...
        <div className="row">
          ...
          <div className="col-md-10">
            <CardHeader sx={{ px: 2, py: 1 }} title={title}
              subheader={`${title == 'All Learning Path' ? assignCourse?.length : assignRCourse?.length}/
              ${title == 'All Learning Path' ? learningPath?.length : selectedLP?.length} learningPath Selected`}
            />
          </div>
        </div>
