...
export const FeedbackModal = ({ open, setOpen }) => {
  const [ratings, setRatings] = React.useState(questions.reduce((obj, ques, index) => { obj['q' + index] = -1; return obj; }, {}))
  const [ratingErrors, setRatingErrors] = React.useState(questions.reduce((obj, ques, index) => { obj['q' + index] = false; return obj; }, {}))
  ...
  const submitFeedback = () => {
    setRatingErrors(prev => (questions.reduce((obj, ques, index) => { obj['q' + index] = false; return obj; }, {})))
  ...
  function remindMeLater() {
    ...
    localStorage.setItem("feedback_form", JSON.stringify({ next_reminder: after.format() }));
    handleClose();
  }
        ...
        <Box sx={style}>
          <div className='feedback-container'>
            ...
            <div className='feedback-header'>
              <figure className='logo-block'>
                <img src={logoImage} alt='' />
              </figure>
              <div className='title'>
                Your Feedback helps us to improve!
              </div>
              <div class="icon icon-cross cancel-icon" onClick={handleClose}></div>
            </div>
            ...
            <div className='feedback-body'>
              <div className='questions-ratings-block'>
                {questions.map((ques, index) => (
                  <div className='question-block' key={index}>
                    <p className='question'> {ques} *
                      {ratingErrors['q' + index] && <p className='error-msg'> {rating_error_msg} </p>}
                    </p>
                    <div className='rating-block'>
                      <CustomizedRating
                        value={ratings['q' + index]}
                        onChange={(event, newValue) => {
                          setRatings((prev) => ({ ...prev, ['q' + index]: newValue }));
                        }}
                        key={index}
                        name={'unique' + index}
                      />
              ...
              <div className='suggestions-block'>
                <p className='suggest-title'>Your Suggestions *
                  {suggestionError && <p className='error-msg'> {suggestion_error_msg} </p>}
                </p>
                <div className='suggest-box'>
                  <textarea placeholder='Type here ...' minlength="60" required
                    value={suggestion}
                    onChange={(e) => setSuggestion(prev => e.target.value)}
                  />
                  {suggestion?.length <= 60 ? <p className='min-character'> Minimum 60 characters ({suggestion?.length} / 60)</p> : ""}
                </div>
              </div>

              <div className='close-btns'>
                <input type='button' value='Remind me later' onClick={remindMeLater} />
          ...
          >
            <div className={toastStatus ? 'toast-success' : 'toast-error'}>
              <span>{toastStatus ? <ThumbUpAltIcon /> : <ErrorOutlineIcon />} </span>
              <span>{toastStatus ? 'Feedback Form Submitted Successfully' : 'Error in Form Submission'} </span>
            </div>
          </Snackbar>
        </Box>
      </Modal>
