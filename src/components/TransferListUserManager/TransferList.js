...
const TransferList = (props) => {
  const { selectedUsers, setSelectedUsers, users, setUsers, setSearchedUser, searchedUser, setBtnLoading, setOpen, setMessage, setSeverity } = props;
  ...
  const handleLeftToggle = (value, type) => () => {
    ...
    if (isSelected) {
      ...
      if (assignCourse?.length > 0) {
        setBtnLoading(false);
        setOpen(true);
        setMessage("Only one manager can Selected");
        setTimeout(() => {
          setOpen(false);
        }, 3000);
        setSeverity("error");
  ...
  const handleCheckedRight = async () => {
    ...
    setAssignCourse([])
    setUsers([...rmainCourses, ...selectedUsers])
  };

  const requestSelectSearch = (searchText) => {
    setAllSelectedCourse(selectedUsers.length ? selectedUsers?.filter(val => val.title_txt.toLowerCase().includes(searchText.toLowerCase())) : [])
  }
    ...
    <>
      <div className="inner-box">
        <div className="row">
          ...
          <div className="col-md-10">
            <CardHeader sx={{ px: 2, py: 1 }} title={title}
              subheader={`${title == 'All Managers' ? assignCourse?.length : assignRCourse?.length}/${title === 'All Managers' ? users?.length : selectedUsers?.length
                } managers Selected`}
            />
          </div>
        </div>
        <div className="search-bar">
          {title === 'All Managers' ?
            <input type="text" placeholder="Search manager and add" onChange={(e) => { setSearchedUser(e.target.value) }} />
            : <input type="text" placeholder={"Search manager and add"} onChange={e => requestSelectSearch(e.target.value)} />
          }
        </div>
          ...
          {items?.length > 0 && items?.map((value) => {
            ...
            return (
              <ListItem key={value.id + value.type} role="listitem" button
                onClick={title === 'All Managers' ? handleLeftToggle(value, title) : handleRightToggle(value, title)}
              >
                <ListItemIcon>
                  <Checkbox
                    checked={
                      title === 'All Managers' ? assignCourse?.some(item => item.id === value.id && item.type === value.type) :
                  ...
                  <div className="course-img">
                    <figure>
                      <img className="img-full" alt=''
                        src={value?.src ? process.env.REACT_APP_WEB_MEDIA_URL + value?.src?.replace("media/", "") : images.user_img} />
                    </figure>
                    {value?.firstname &&
                      <div>
  ...
  return (
    <div className="transfer-list">
      <div className="list-box right">{customList("All Managers", users)}</div>
      ...
      <div className="list-box left">
        {customList("Selected Manager", allSelectedCourse)}
      </div>
    </div>
  );
};
