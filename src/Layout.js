...
const Layout = ({ children, mainClass }) => {
  ...
  //   JSON.parse(localStorage.getItem("releaseNotesModal")).modal
  // );
  ...
  if (privileges?.is_employee) {
    ...
    let privateRenderRoutesValues = Object.fromEntries(
      Object.entries(privateRenderRoutes).map(
        (el) => ((el[1] = Object.values(el[1])[1]["path"].split("/")[1] == pathnameForConditon
          ? Object.values(el[1])[1]["menuname"] : ""), el)
      )
    );
    let cleanObj = {};
    ...
    if (privileges?.is_owner || (privileges?.privileges.includes(cleanObjFinal[0]))) {
    } else {
  ...
  useEffect(() => {
    ...
    if (JSON.parse(localStorage.getItem("feedback_form"))?.next_reminder) {
      const nextReminder = moment(JSON.parse(localStorage.getItem("feedback_form"))?.next_reminder);
  ...
  useEffect(()=>{
    if(expiryDate){
      if(moment().format("YYYY-MM-DD HH:mm:ss") > moment(expiryDate).format("YYYY-MM-DD HH:mm:ss")){
    ...
    <>
      {!loading && privileges != 0 ? (
        <>
          ...
          {feedbackModal ? <FeedbackModal setOpen={setFeedbackModal} open={feedbackModal} /> : ""}
          <div className={`wrapper ${mainClass || ""}`}>
            <Maintenance />
            <Header privileges={privileges} setFeedbackModal={setFeedbackModal} />
            <div id="content-area">
